...
function KanbanColumn({ id, naam, vehicles, onDrop, onSelectVehicle }: KanbanColumnProps) {
  ...
  return (
    <div
      className="flex-shrink-0 w-[260px] sm:w-[300px] bg-muted/50 rounded-xl border border-border"
    ...
    >
      ...
      <div className="p-2 space-y-2 max-h-[55vh] overflow-y-auto">
        ...
        {vehicles.map((v) => (
          <VehicleKanbanCard key={v.id} vehicle={v} onClick={() => onSelectVehicle?.(v)} />
        ))}
      </div>
    </div>
  );
...
function VehicleKanbanCard({ vehicle, onClick }: { vehicle: Vehicle; onClick?: () => void }) {
  ...
  return (
    <div
      ...
      onClick={onClick}
      className={cn(
        "clean-card p-3 cursor-grab active:cursor-grabbing hover:shadow-md transition-all animate-fade-in"
      )}
    >
