...
BEGIN
  IF TG_TABLE_NAME = 'contracts' THEN
    ...
    EXCEPTION WHEN others THEN
      ...
      LIMIT 1;
    END;
    -- Probeer gekoppelde reservering te halen uit notities (formaat: 'reservering:<uuid>')
    BEGIN
      v_skip_reservering := (regexp_match(COALESCE(NEW.notities,''), 'reservering:([0-9a-fA-F-]{36})'))[1]::uuid;
    EXCEPTION WHEN others THEN
      v_skip_reservering := NULL;
    END;
  ELSE
    v_voertuig_uuid := NEW.voertuig_id::uuid;
  END IF;
  ...
  WHERE r.voertuig_id = v_voertuig_uuid
    ...
    AND NOT (TG_TABLE_NAME = 'contracts' AND v_skip_reservering IS NOT NULL AND r.id = v_skip_reservering)
