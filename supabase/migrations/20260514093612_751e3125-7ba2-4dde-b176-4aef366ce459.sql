...
SET status = 'concept'
WHERE c.notities LIKE '%Automatisch aangemaakt vanuit reservering:%'
  AND c.status = 'actief'
  AND NOT EXISTS (
    SELECT 1 FROM public.overdrachten o
    WHERE o.contract_id = c.id AND o.type = 'ophalen' AND o.status = 'ondertekend'
  );
...
SELECT c.user_id, c.organisatie_id, c.id, c.voertuig_id,
       COALESCE(v.kenteken, 'Onbekend'),
       COALESCE(v.merk || ' ' || v.model, 'Voertuig'),
       c.klant_naam, c.klant_email, 'ophalen', c.start_datum, 'wacht_op_handtekening'
FROM public.contracts c
LEFT JOIN public.voertuigen v ON v.id::text = c.voertuig_id
WHERE c.notities LIKE '%Automatisch aangemaakt vanuit reservering:%'
  AND c.status IN ('concept','actief')
  AND NOT EXISTS (
    SELECT 1 FROM public.overdrachten o
